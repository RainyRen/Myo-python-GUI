...
class MANNCell:
    ...
    def __call__(self, x, prev_state):
        ...
        # state of controller (LSTM hidden state)
...
class MANNCellA:
    def __init__(self, rnn_size, memory_size, memory_vector_dim, head_num, gamma=0.95,
        ...
        self.head_num = head_num                                    # #(read head) == #(write head)
        self.reuse = reuse
        self.kin_controller = tf.nn.rnn_cell.BasicLSTMCell(self.rnn_size)
        self.emg_controller = tf.nn.rnn_cell.BasicLSTMCell(self.rnn_size)
        self.step = 0
        self.gamma = gamma
        self.k_strategy = k_strategy
    ...
    def __call__(self, x_kin, x_emg, x_label, prev_state):
        # read vector (the content that is read out, length = memory_vector_dim)
        prev_read_vector_list = prev_state['read_vector_list']
        # state of controller (LSTM hidden state)
        prev_kin_controller_state = prev_state['kin_controller_state']
        prev_emg_controller_state = prev_state['emg_controller_state']

        # x + prev_read_vector -> controller (RNN) -> controller_output

        # controller_input = tf.concat([x] + prev_read_vector_list, axis=1)
        with tf.variable_scope('controller', reuse=self.reuse):
            kin_controller_output, kin_controller_state = self.kin_controller(x_kin, prev_kin_controller_state)
            emg_controller_output, emg_controller_state = self.emg_controller(x_emg, prev_emg_controller_state)
            merge_input = tf.concat(
                [kin_controller_output, emg_controller_output, x_label] + prev_read_vector_list, axis=1)

        controller_output = tf.contrib.layers.fully_connected(merge_input, 64, activation_fn=None, reuse=self.reuse)
        ...
        state = {
            'kin_controller_state': kin_controller_state,
            'emg_controller_state': emg_controller_state,
            'read_vector_list': read_vector_list,
            'w_r_list': w_r_list,
            'w_w_list': w_w_list,
            'w_u': w_u,
    ...
    def zero_state(self, batch_size, dtype):
        ...
        with tf.variable_scope('init', reuse=self.reuse):
            state = {
                'kin_controller_state': self.kin_controller.zero_state(batch_size, dtype),
                'emg_controller_state': self.emg_controller.zero_state(batch_size, dtype),
                'read_vector_list': [tf.zeros([batch_size, self.memory_vector_dim])
                                     for _ in range(self.head_num)],
                'w_r_list': [one_hot_weight_vector for _ in range(self.head_num)],
                'w_u': one_hot_weight_vector,
